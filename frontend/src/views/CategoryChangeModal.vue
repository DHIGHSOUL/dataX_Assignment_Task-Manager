<template>
    <div class="modal-overlay" @click.self="close">
        <div class="modal-content">
            <h1>カテゴリ変更</h1>
            <p class="update-label">変更するカテゴリ情報を入力してください。</p>
            <form>
                <div class="category-item-group">
                    <p class="update-label">カテゴリ名</p>
                    <input class="name-input" v-model="categoryName" placeholder="カテゴリ名" type="text" required />
                </div>
                <div class="category-item-group">
                    <p class="update-label">カテゴリのカラーコード</p>
                    <input class="color-input" v-model="categoryColor" placeholder="カテゴリの色" type="text" />
                </div>
                <div class="category-change-button-group">
                    <button type="button" class="update-button" @click="updateCategory">変更</button>
                    <button type="button" class="delete-button" @click="deleteCategory">削除</button>
                </div>
...
const fecthOriginalCategoryInfo = async () => {
    try {
        const response = await axios.get(`/api/workspace_categories/${props.categoryID}`)
