    ...
    .modal-content-outside
      .modal-content
        ...
        form
          .task-item-group
            p.update-label タスク名
            input.name-input(v-model="taskName" placeholder="タスク名" type="text" required)
          .task-item-group
            p.update-label タスクの説明
            textarea.description-input(v-model="taskDescription" placeholder="タスクの説明(Option)" type="text" @input="autoResize" ref="descriptionRef")
          .task-item-group
            p.update-label タスクの期限
            input.due-date-input(v-model="dueDate" type="date" required)
          .task-item-group
            p.update-label タスクの進行状況
            Multiselect.status-input(v-model="selectedStatus" :options="statusOptions" label="text" track-by="value" placeholder="タスクの進行状況を選択してください")
          button.update-button(type="button" @click="updateTask") 更新
          button.cancel-button(type="button" @click="close") キャンセル
      .category-and-assignee-group
        form
          .task-item-group
            p.update-label タスクのカテゴリ
          ...
          .task-item-group-assignee
            p.update-label タスクの担当者を選択してください。
...
const fetchOriginalTaskInfo = async () => {
  try {
    ...
    selectedStatus.value =
      statusOptions.find((s) => s.value === originalStatus) ?? null;
    const originalCategory = response.data.task.workspace_category;
    if (originalCategory) {
      selectedCategory.value =
        categories.value.find((c) => c.id === originalCategory.id) ?? null;
    } else {
...
const fetchOriginalAssignees = async () => {
  try {
    ...
    assignees.value = users.value.filter((user) => {
      return originalAssignees.value.some(
...
const updateTaskAssignments = async () => {
  ...
  await Promise.all(
    toRemoveIDs.map(async (id) => {
      ...
      try {
        await axios.delete(
          `/api/tasks/${taskID}/task_assignments/${assignment.id}`
        );
      } catch (error) {
